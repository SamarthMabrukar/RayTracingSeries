...
private:
    ...
    void QueryGPUFeatures()
    {
        ...
        __try {
            ...
            __except(EXCEPTION_EXECUTE_HANDLER) {
                StringCchCopyW(m_szAdapterName, 256, L"Unknown (DXGI error)");
            }
            ...
            __try {
                ...
                if(hr == E_INVALIDARG)
                {
                    // Runtime doesn't recognize SM 6.9, try 6.8
                    smData.HighestShaderModel = D3D_SHADER_MODEL_6_8;
                    hr = pDevice->CheckFeatureSupport(D3D12_FEATURE_SHADER_MODEL, &smData, sizeof(smData));
            ...
            __except(EXCEPTION_EXECUTE_HANDLER) {
                StringCchCopyW(m_szLastError, 512, L"Exception during shader model query");
            }
