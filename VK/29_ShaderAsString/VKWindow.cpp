...
VkResult CreateShaders(void)
{
	BOOL glsl2spv(const VkShaderStageFlagBits, const char*, std::vector<unsigned int>&);
	...
	const char* vertexShaderSrcCode = "#version 450 core\n" \
		...
		"}";
	std::vector<unsigned int> vertexShader_spv;
	bDone = glsl2spv(VK_SHADER_STAGE_VERTEX_BIT, vertexShaderSrcCode, vertexShader_spv);
	if (bDone == false)
	{
		fprintf(g_pFile, "CreateShaders -> glsl2spv(Vertex Shader) Failed.\n");
		return (VK_ERROR_INITIALIZATION_FAILED);
	}
	...
	vkShaderModuleCreateInfo.codeSize = sizeof(unsigned int) * vertexShader_spv.size();
	vkShaderModuleCreateInfo.pCode = (uint32_t*)vertexShader_spv.data();
	...
	// For Fragment Shader
	const char* fragmentShaderSrcCode = "#version 450 core\n" \
		"#extension GL_ARB_separate_shader_objects : enable\n" \
		...
		"}";
	std::vector<unsigned int> fragmentShader_spv;
	bDone = glsl2spv(VK_SHADER_STAGE_FRAGMENT_BIT, fragmentShaderSrcCode, fragmentShader_spv);
	if (bDone == false)
	{
		fprintf(g_pFile, "CreateShaders -> glsl2spv(Fragment Shader) Failed.\n");
		return (VK_ERROR_INITIALIZATION_FAILED);
	}
	...
	vkShaderModuleCreateInfo.pNext = NULL;
	vkShaderModuleCreateInfo.flags = 0;// Reserved for future use.
	vkShaderModuleCreateInfo.codeSize = sizeof(unsigned int) * fragmentShader_spv.size();
	vkShaderModuleCreateInfo.pCode = (uint32_t*)fragmentShader_spv.data();
...
BOOL glsl2spv(const VkShaderStageFlagBits shaderType, const char* pShaderSrc, std::vector<unsigned int>& spirv)
{
	// Function Declarations
	...
	// Create TShader type shader from above EShLanguage by calling Tshader's constructor
	glslang::TShader shader(language);

	// Put our shader Source cod into a string array
	const char* shaderStrings[1];

	shaderStrings[0] = pShaderSrc;

	// Initialize built in shader resources
	const TBuiltInResource *builtInResource = GetDefaultResources();
	//InitBuiltinResources(builtInResource);
	...
	// Parse the Tshader to Compile.
	if (!shader.parse(builtInResource, 100, false, ruleMessages))
	{
		fprintf(g_pFile, "glsl2spv -> glslang::TShader.parse() Failed with log %s.\n", shader.getInfoLog());
		fprintf(g_pFile, "glsl2spv -> glslang::TShader.parse() Failed with debug log %s.\n", shader.getInfoDebugLog());
		return (retVal);
	}
	else
	{
		fprintf(g_pFile, "glsl2spv -> glslang::TShader.parse() Successful.\n");
	}

	// Add shader to TProgram, can't customize the program as there is no parameterized constructor
	...
	{
		fprintf(g_pFile, "glsl2spv -> glslang::TProgram.link() Failed with log %s.\n", shader.getInfoLog());
		fprintf(g_pFile, "glsl2spv -> glslang::TProgram.link() Failed with debug log %s.\n", shader.getInfoDebugLog());
		return (retVal);
	}
	else
	{
		fprintf(g_pFile, "glsl2spv -> glslang::TProgram.link() Successful.\n");
	}

	// set this whole cooked program and spirv to glslang library
...
EShLanguage FindShaderLanguage(const VkShaderStageFlagBits shaderStage)
{
	switch (shaderStage)
	{
